...
- **Halo 2.26 及以上**（按官方 2.26 文档与扩展点实现；Java 17 字节码；全新 2.26 实例全量回归 50/50 通过）
...
构建结果位于 `build/libs/plugin-important-dates-<版本>.jar`（当前版本：`1.2.2`）。

> 版本说明：**最新正式版** `1.2.2`（修复附件库「分类 / 策略」来源范围：不再因二者同时不命中而显示空白，「未分组」正确匹配，未配置时不再自动收窄；并含 `1.2.1` 的座驾到期项改造：车辆级保险公司、循环间隔、年检自动推算与手动覆盖、年检/车船税与保险同期、同车同日合并提醒、仪表盘小组件分页与底部条数）。上一个正式版为 `1.2.0`（更名「记得」，新增座驾模块）。装包后请强刷一次浏览器（Ctrl+Shift+R）。
