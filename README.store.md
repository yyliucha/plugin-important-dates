...
构建结果位于 `build/libs/plugin-important-dates-<版本>.jar`（当前版本：`1.2.4`）。

> 版本说明：**最新正式版** `1.2.4`（修复拖拽排序保存时的 503：改为一处必要的 JSON Patch、串行提交并带退避重试，不再产生被反向代理限流的请求突发；并含 `1.2.3` 的座驾卡片样式修复、`1.2.2` 的附件库来源范围修复、`1.2.1` 的座驾到期项改造）。更早的正式版为 `1.2.0`（更名「记得」，新增座驾模块）。装包后请强刷一次浏览器（Ctrl+Shift+R）。
