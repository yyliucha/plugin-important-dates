...
- **到期项「已办，顺延一期」**：续保 / 年检办完后一键把到期日按循环间隔往后推一期，不用手改日期
...
- **前台爱车视图工具栏**：按「默认（后台拖拽顺序）/ 到期临近 / 名称」排序，按「在用车 / 全部（含已出售、已报废）」筛选
- **后台「自检」面板**：页面右上角一键查看插件版本、数据量、提醒与展示设置、照片设置命中张数、图片引用失效数量、即将到期条数
...
构建结果位于 `build/libs/plugin-important-dates-<版本>.jar`（当前版本：`1.2.5`）。

> 版本说明：**最新正式版** `1.2.5`（14 项完善：写操作统一为「只改变化字段 + 串行 + 退避重试」、导入节流与失败明细、中文可操作报错、上传后自检、图片失效自动降级与一键修复、缩略图与上传压缩、到期项「已办顺延一期」、提醒逐条忽略、年检规则可配置、爱车排序筛选、后台自检面板、设置「关于」分组；并含 `1.2.2` 附件库范围修复、`1.2.3` 卡片样式修复、`1.2.4` 拖拽 503 修复）。更早的正式版为 `1.2.0`（更名「记得」，新增座驾模块）。装包后请强刷一次浏览器（Ctrl+Shift+R）。
