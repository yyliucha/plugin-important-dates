...
> 版本说明：**最新正式版** `1.2.6`（提醒状态机：已办才滚动、逾期 3 天逐日提醒后转待处理、节点式提醒每 7 天一次、保养项「已办」= 刚做完保养、一键撤销、前台只显示未逾期的待办项、弹窗内动作原地生效、提示语全面人性化；并含 `1.2.5` 的写操作加固与图片失效降级、`1.2.4` 拖拽 503 修复、`1.2.3` 卡片样式修复、`1.2.2` 附件库范围修复）。装包后请强刷一次浏览器（Ctrl+Shift+R）。
