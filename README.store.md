...
构建结果位于 `build/libs/plugin-important-dates-<版本>.jar`（当前版本：`1.2.3`）。

> 版本说明：**最新正式版** `1.2.3`（修复后台「座驾」页签的样式丢失问题：约 3KB CSS 曾被构建丢弃，导致相册有照片后封面铺满屏幕——现已恢复紧凑卡片布局与多列网格；并含 `1.2.2` 的附件库来源范围修复、`1.2.1` 的座驾到期项改造）。更早的正式版为 `1.2.0`（更名「记得」，新增座驾模块）。装包后请强刷一次浏览器（Ctrl+Shift+R）。
