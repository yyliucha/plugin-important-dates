...
- **Dashboard widget paging**: 5 items per page by default, adjustable or switchable off under Settings → Display; the widget shows the **complete reminder list** (no longer capped by the toast noise limit), always reports the total ("9 items · 1 per page") and paging only changes local state
