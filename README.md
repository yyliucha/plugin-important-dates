...
构建结果位于 `build/libs/plugin-important-dates-<版本>.jar`（当前版本：`1.2.1-SNAPSHOT`）。

> 版本说明：**最新正式版** `1.2.0`（更名「记得」，新增座驾模块）；**当前开发版 `1.2.1-SNAPSHOT`**（座驾到期项改造：车辆级保险公司、循环间隔、年检自动推算与手动覆盖、年检/车船税与保险同期、同车同日合并提醒、仪表盘小组件分页；并含 `1.2.1` 号段此前未发布的修复：无相册座驾点击提示、附件库范围与加载、大头贴/座驾相册设置拆分、菜单图标、皮卡图标、共同持有配色）。测试通过后发布正式版 `1.2.1`（该号段此前未发布过 Release，故不跳号）。
>
