...
- **控制台仪表盘小组件**（Halo 2.21+ 控制台）：插件注册「📅 重要日期提醒」仪表盘小部件，到期提醒**常驻仪表盘、一眼可见、无关闭按钮**，每 60 秒自动刷新。添加一次即可：仪表盘 →「编辑仪表盘」→「添加部件」→「小部件中心」→ 分组 `plugin-important-dates` → 点击「重要日期提醒」卡片 →「保存」；旧版本 Halo 控制台自动忽略（后台提醒仍走插件页顶部横幅）
...
行为：访问任意页面时按所选位置浮出提醒；按「提醒设置 → 悬浮提示自动关闭秒数」自动关闭（默认 8 秒），**自动关闭时弹窗内显示倒计时文字与进度条**；倒计时/点 × 只收起当前弹窗，刷新/换页重新弹出。**关闭行为可配置**（见下）。
- 点 × 弹出「关闭方式」菜单：**本次关闭**（仅收起当前弹窗，刷新/换页重新弹出）/ **3 天内不显示** / **10 天内不显示** / **永久关闭**；5 秒未选择按设置里的「× 关闭默认行为」执行
- 站主设置：**× 关闭默认行为**（本次 / 1 天 / 3 天 / 7 天 / 10 天 / 30 天 / 永久）+ **关闭方式菜单**开关（关掉后点 × 直接按默认行为执行）
- 时长与永久记录在访客浏览器 localStorage（无痕窗口独立；清浏览器站点数据可重置）
...
构建结果位于 `build/libs/plugin-important-dates-1.0.30.jar`。
